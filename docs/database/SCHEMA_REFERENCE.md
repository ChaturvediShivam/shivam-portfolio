...
|-------|:------:|---------|----------|-----------------|---------|-----|-------------|-----------------|----------|-----------|
| `automation_rules` | ✅ M10 | User rules (trigger→cond→action) | `id`; `owner_id` | `name`, `trigger jsonb`, `conditions jsonb`, `actions jsonb`, `enabled` (default **false**), `last_scheduled_at` | `(enabled) where not archived`, `(owner_id, created_at desc)` | standard | `archived_at` | — | engine reads | M10 |
| `automation_runs` | ✅ M10 | Per-evaluation audit **and loop governor** | `id`; `rule_id → automation_rules CASCADE` | `trigger_type`, `event_type`, `entity_type/id`, `status`, `reason`, `action_results jsonb`, `error`, `idempotency_key` | `(rule_id, created_at desc)`, `(created_at desc)`, `(rule_id, entity_id, created_at desc)`, unique `idempotency_key` | standard | none (append) | `automation.run_*`, `action_executed` | engine writes | M10 |

- **Audit:** every *evaluation* is recorded in `automation_runs`, including the ones that matched nothing — the table is simultaneously the audit trail and the loop governor, and a governor that only counted successes would not bound a rule whose actions keep failing. Entity-affecting actions also write `opportunity_events` (`actor_type='agent'`). **Triggers:** `updated_at` on both tables.
- **Loop safety:** `(rule_id, entity_id, created_at desc)` serves the governor's count, which runs before every execution and so must not scan history. The unique index on `idempotency_key` is claimed *before* actions execute, turning the job queue's at-least-once delivery into exactly-once execution.
